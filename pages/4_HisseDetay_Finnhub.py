...
# ---------------- Veri çek -------------------
# API key (güvenli saklama için çevresel değişkenle önerilir ama sabit yazabilirsin test için)
api_key = "d23bhm9r01qgiro2uqbgd23bhm9r01qgiro2uqc0"
finnhub_client = finnhub.Client(api_key=api_key)

# Hisse simgesini FINNHUB formatına dönüştür (örneğin THYAO.IS → THYAO.IS, AAPL → AAPL)
symbol = hisse.upper()

# Tarihleri epoch cinsine çevir
import time
from datetime import datetime, timedelta

end_time = int(time.time())
start_time_map = {
    "1d": end_time - 60 * 60 * 24,
    "5d": end_time - 60 * 60 * 24 * 5,
    "1mo": end_time - 60 * 60 * 24 * 30,
    "3mo": end_time - 60 * 60 * 24 * 90,
    "6mo": end_time - 60 * 60 * 24 * 180,
}
start_time = start_time_map.get(zaman_araligi, end_time - 60 * 60 * 24 * 30)

# Veri çek
try:
    res = finnhub_client.stock_candles(symbol, "D", start_time, end_time)
    if res["s"] != "ok":
        st.error("Veri alınamadı. Hisse kodu yanlış olabilir.")
        st.stop()
